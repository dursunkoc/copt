...
def eligibility(e_cu, X, c, u, h, d):
...
def check(X, PMS, indicies):
    return cstr.check_indicies(X,PMS.b, PMS.cuhd, PMS.e_cu, PMS.k, PMS.l_c, PMS.m_i, PMS.n_i, PMS.q_ic, PMS.s_cuhd, PMS.t_hd,indicies)

def check_no_rh(X, PMS, indicies):
    c_i = 0
    u_i = 1
    h_i = 2
    d_i = 3
    if not eligibility(PMS.e_cu, X, indicies[c_i],indicies[u_i],indicies[h_i],indicies[d_i]):
        return False
    if not weekly_limitation(PMS.b, X, indicies[u_i]):
        return False
    if not daily_limitation(PMS.k, X, indicies[u_i],indicies[d_i]):
        return False
    if not campaign_limitation(PMS.l_c, X, indicies[c_i],indicies[u_i]):
        return False
    if not weekly_quota(PMS.m_i, PMS.q_ic, X, indicies[u_i]):
            return False
    if not daily_quota(PMS.n_i, PMS.q_ic, X, indicies[u_i],indicies[d_i]):
        return False
    if not channel_capacity(PMS.t_hd, X, indicies[h_i],indicies[d_i]):
        return False
    return True
...
def interaction_matrix(X, a_uv):
    X_cu = X.sum(axis=(2,3))
    return (np.matmul(X_cu, a_uv )+X_cu > 0).astype(int)

def objective_fn(rp_c, X, a_uv):
    Y_cu = interaction_matrix(X, a_uv)
...
def X_eligibility (e_cu_X, X):
    return (X <= e_cu_X).all()
def X_weekly_limitation (b, X):
    return (X.sum(axis=(0,2,3))<=b).all()
def X_weekly_limitation_rh (b, X):
    return (X.sum(axis=(0,2,3))<=b).all()
def X_daily_limitation (k, X):
    return (X.sum(axis=(0)).sum(axis=(1))<=k).all()
def X_campaign_limitation (l_c, X):
    return np.all(X.sum(axis=(2,3)).T<=l_c)
def X_weekly_quota (m_i, q_ic, X):
    ...
    for i in q_range:
        ...
        if not quota_i:
            return False
    return True
def X_channel_capacity (t_hd, X):
    return np.all(X.sum(axis=(0,1))<=t_hd)

def X_daily_quota (n_i, q_ic, X):
    q_range = range(q_ic.shape[0])
    for i in q_range:
...
def X_check(PMS, X):
    if not X_eligibility(PMS.e_cu_X, X):
        return False
    if not X_channel_capacity(PMS.t_hd, X):
        return False
    return True
