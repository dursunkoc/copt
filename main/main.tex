...
\subsection{Modifications in the current model} \label{s:network-modif-model}
...
\subsection{Building A Network Using Tariff-change Data and Social Network}\label{s:micro-world-networks}
...
\\
\item Case 2: Number of Customers: 10, Random Seed: 45, Network Type: barabasi, M:1, Edge Drop Propability: \%10.
\\
\begin{figure}[htp]
    \centering
    \includegraphics[width=6cm]{micro_world_case_2}
    \caption{Network for Case 2}
...
\\
\item Case 3: Number of Customers: 20, Random Seed: 83, Network Type: barabasi, M:2, Edge Drop Propability: \%80.
\\
\begin{figure}[htp]
    \centering
    \includegraphics[width=6cm]{micro_world_case_3}
    \caption{Network for Case 3}
...
\\
\item Case 4: Number of Customers: 10, Random Seed: 42, Network Type: erdos, p:\%8.
\\
\begin{figure}[htp]
    \centering
    \includegraphics[width=6cm]{micro_world_case_4}
    \caption{Network for Case 4}
...
\\
\item Case 5: Number of Customers: 10, Random Seed: 121, Network Type: erdos, p:\%4.
\\
\begin{figure}[htp]
    \centering
    \includegraphics[width=6cm]{micro_world_case_5}
    \caption{Network for Case 5}
...
\item Case 6: Number of Customers: 20, Random Seed: 142, Network Type: erdos, p: %3
Network.
\\
\begin{figure}[htp]
    \centering
    \includegraphics[width=6cm]{micro_world_case_6}
    \caption{Network for Case 6}
...
\end{itemize}
\subsection{Heuristics to Solve Campaign Optimization Problem With Network \label{s:heu-camp-opt-with-net} Effect}\label{s:micro-world-networks}
From the cases at \S \ref{s:micro-world-networks}, we have concluded with the following algorithm-\ref{algo:cust-sort-a} which sorts nodes by their degree.
\begin{algorithm}[H]
...
In this algorithm we sort customers by their degree centrality in the social network graph. In order to validate this algorithm we generated the following greater network than the ones at \S \ref{s:micro-world-networks}.
...
\end{figure}
\\
The mathmatical modeal and optimal solution for the network is presented at appendix \ref{s:apendix-mini-net-cplex-sol}. The optimal solutions objective value is 348, however the objective value is 324 if we apply the algorithm-\ref{algo:cust-sort-a}.\\
Our algorithm-\ref{algo:cust-sort-a}, selects a random customer when two customers degree centrality is equal, however selecting the one whose neighbours have not been selected would be much better. So we developed algorithm-\ref{algo:cust-sort-b} which pushes back a customer if that customers neighbour already selected.
...
Using algorithm-\ref{algo:cust-sort-b} we attained an objective value of 336. 
\\
Finally we developed algorithm-\ref{algo:cust-sort-c} which uses mathematical model described below;
Following mathematical modal finds the customers which are sufficient and necessary to cover the whole network.
\\
\noindent \textbf{Sets}\\
...
\noindent $W_{u}=1$, if customer $u \in \mathcal{U}$ is sufficient and necessary, and 0 otherwise.
...
The objective function \eqref{mathmodel_net_cov_obj} minimizes the number of nodes to be selected. Constraints \eqref{mathmodel_net_cov_s1} ensure that each customers or its level-1 neighbour is touched, constraints \eqref{mathmodel_net_cov_s2} ensure that a customer is touched if it is eligible; and finally constraint \eqref{mathmodel_net_cov_integrity} ensure that the variable $W_{u}$ is either 0 or 1.\\
...
\begin{algorithm}[H]
\KwIn{G graph of customer network}
\KwIn{W sufficient and necessary customers}
\KwOut{A Sorted list of customers $SC$}
\State $SC \gets []$
\State $SS \gets []$
...
Using algorithm-\ref{algo:cust-sort-c} we attained an objective value of 348 which is equal to the optimal solution. 
