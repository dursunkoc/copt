...
In this study, we plan to develop a model which tries to maximize the number of interactions and maximize the expected return on the messages sent to customers via different channels such as SMS, push-notifications, call-centers and IVR%{Bu kisimi duzeltmemiz gerekiyor.}, while minimizing the cost of channel usage subject to the availability of channels during the day, and confirming the right time for customer interests. There will be two main types of data; call and content. We plan to obtain them from Turkcell. Data analysis will be done by statistical and machine learning techniques. Call data will provide us the basic network. Target audience will be shrunk down using network analysis techniques, by keeping the most influential customers on the target list and removing the ones who can be targeted indirectly. Identifying the right channel and the right time for each customer in the target audience will be achieved using optimization techniques. We will develop heuristic methods to solve this difficult problem and after thorough computational experiments we plan to produce managerial insights that can be generalized for similar communication networks.
...
% {\color{blue}genel model icin aciklama gelmeli}The objective function \eqref{recobj} minimizes total recovery time of all nodes. Constraints \eqref{comp_2} ensure that the recovery time of a node can occur after all the broken (damaged) nodes that it depends on have been repaired. Constraints \eqref{comp_new} define the recovery time of a node based on the recovery times of other nodes by considering the interdependencies between them. Consequently, if the recovery of a damaged node $i$ does not depend on any other node, then the recovery time of $i$ occurs once it is repaired. However, if recovery of node $i$ depends on other nodes, then it is fully recovered once all $j$ $(i\neq j)$ for which $d_{ij}=1$ are recovered. Arrival times of the repair teams to the first visited damaged nodes of power and gas networks are defined by using Constraints \eqref{first_parrival} and \eqref{first_garrival}, respectively. Constraints \eqref{parrival} and \eqref{garrival} calculate the arrival times to the other damaged nodes. In these constraints, $M_{p}$ and $M_{g}$ values are set for specific instance by computing a loose upper bound for the node arrival times. Specifically, for each infrastructure, we multiply the largest travel time in the corresponding logistics network by the number of damaged nodes in the infrastructure and add the sum of repair time for all damaged nodes except the smallest repair time; for instance, $M_{p}$ = $\displaystyle\max_{i,j\in {N}_{p}} t_{ij} \times |B_p| + \sum\limits_{i\in B_p} r_{i} - \min_{i\in B_p} r_{i}$. Constraints \eqref{v_power} and \eqref{v_gas} limit the number of vehicles leaving the repair center. Constraints \eqref{prepairdamaged} and \eqref{grepairdamaged} guarantee  that one vehicle leaves each visited damaged node, and Constraints \eqref{backprepairdamaged} and \eqref{backgrepairdamaged} ensure one vehicle arrives at each damaged node. Constraints \eqref{parrival} and \eqref{garrival} along with Constraints \eqref{prepairdamaged}-\eqref{backgrepairdamaged} prevent subtours. Note that since the objective function minimizes total recovery  time, and all recovery times can be computed once the last damage node is repaired, we are not necessarily interested in a vehicle's return to the repair center. That is, although we construct closed routes that return to the repair center, the length of the last travelled link does not affect the objective function. Lastly, \eqref{nonneg1}, \eqref{nonneg2} and \eqref{xij} define variable domains. 

%The objective function \eqref{recobj_new} minimizes total recovery time of all nodes. Constraints \eqref{comp_2_new} ensure that the recovery time of a node can occur after all the broken (damaged) nodes that it depends on have been repaired. Constraints \eqref{comp_new_new} define the recovery time of a node based on the recovery times of other nodes by considering the interdependencies between them. Consequently, if the recovery of a damaged node $i$ does not depend on any other node, then the recovery time of $i$ occurs once it is repaired. However, if recovery of node $i$ depends on other nodes, then it is fully recovered once all $j$ $(i\neq j)$ for which $d_{ij}=1$ are recovered. Note that since the objective function minimizes total recovery  time, and all recovery times can be computed once the last damage node is repaired, we are not necessarily interested in a vehicle's return to the repair center. That is, although we construct closed routes that return to the repair center, the length of the last travelled link does not affect the objective function. Constraints \eqref{first_arrival_new} determine arrival times of the repair teams to the first visited damaged node of each infrastructure network. Constraints \eqref{arrival_new} set the arrival times of the subsequently visited damaged nodes. In these constraints, $M_{l}$ values are set for each instance by computing a loose upper bound for the node arrival times. Specifically, for each infrastructure, we multiply the largest travel time in the corresponding logistics network by the number of damaged nodes in the infrastructure and add the sum of repair times for all damaged nodes except the smallest repair time; that is, $M_{l}$ = $\displaystyle\max_{i,j\in {N}_{l}} t_{ij} \times |B_l| + \sum\limits_{i\in B_l} r_{i} - \min_{i\in B_l} \{r_{i}\}$. Constraints \eqref{repairdamaged_new} and \eqref{backrepairdamaged_new} guarantee that one vehicle leaves and arrives at each visited damaged node, respectively. Constraints \eqref{first_arrival_new}-\eqref{backrepairdamaged_new} prevent subtours. Constraints \eqref{v_new} limit the number of vehicles leaving the repair center for each infrastructure network. Lastly, \eqref{nonneg1_new}-\eqref{xij_new} define the  domains of the variables.
...
%Constraints \eqref{visit_gas} and \eqref{visit_power} prevent visits between networks; i.e., vehicles for power and gas networks may only repair their own networks.  In this way, together with constraints \eqref{prepairdamaged} and \eqref{grepairdamaged}, we formulate a multi-vehicle model without a vehicle index.

% \begin{align}
% s.t. \hspace{1.5cm} 
% &c_{i} \geq d_{ij}(a_{j}+r_{j}) &\forall i \in N, \forall j \in B,\label{comp_2}\\
% %&c_{i} \geq d_{ij}c_{j} &\forall i \in N, &\forall j \in  \{N-B\}\label{comp_3}\\
% &a_{i}\geq \sum\limits_{l\in D} t_{li} x_{li} &\forall i \in B, \label{first_arrival}\\
% &a_{i}+r_{i}+t_{ij} \leq a_{j}+M(1-x_{ij}) &\forall i,j \in B, \label{arrival}\\
% &\sum\limits_{j\in B \cup D} x_{ji}= 1 &\forall i \in B, \label{repairdamaged2} \\
% &\sum\limits_{j\in B \cup D} x_{ij}=1 &\forall i \in B, \label{repairdamaged} \\
% %&\sum\limits_{j\in B \cup D} x_{ij}-\sum\limits_{j\in B \cup D} x_{ji} =0 &\forall i \in B \cup D \label{balance}\\
% &x_{ij} =0 &\forall i \in B_p \cup \{0\},\forall j \in B_g \cup \{1\}, \label{visit_gas}\\
% &x_{ji} =0 &\forall j \in B_g \cup \{1\},\forall i \in B_p \cup \{0\}, \label{visit_power}\\
% &\sum\limits_{j\in B_p} x_{0j}\leq k_p, &\label{v_power}\\
% &\sum\limits_{j\in B_g} x_{1j}\leq k_g, &\label{v_gas}\\
% &c_{i} \geq 0 &\forall i \in N, \label{nonneg1}\\
% & a_{i} \geq 0 &\forall i \in B, \label{nonneg2}\\
% &x_{ij} \in \{0,1\} &\forall i, j \in B \cup D. \label{xij}
% \end{align}
...
%We design a Peer Heuristic $(P)$ for repair scheduling, where we consider dependencies in planning routes for each network while watching over the other network's needs, i.e., peer's considerations are taken into account. Peer Heuristic has two steps. First one is determining damaged node precedences. The second is assigning nodes to vehicles (routes) in a way that minimizes total completion time. We obtain initial routes for the IIRSP at the end of the constructive heuristic and improve these solutions by using local search methods. We provide a short outline of the algorithm below. It is then explained in detail along with illustrative examples. \\


%\noindent\fbox{%
%    \parbox{\textwidth}{%

%\noindent \textbf{Outline of the solution method}\textbf{\hline} %\label{thealgorithm}

%\vspace{0.3cm}

%\noindent \textbf{Constructive Heuristic}\\
%\noindent \textbf{\textit{Step 1}} [Node Prioritization] \\
%Determine which nodes are more critical in terms of the number of other %nodes they affect. 

%\noindent \textbf{\textit{Step 2}} [Assignment to Routes]\\
%Create a route for each vehicle in a way that minimizes the total %completion time. 

%\noindent \textbf{Improvement Method}\\
%\noindent \textbf{\textit{Step 3}} [Local Search]\\
%Reconsider routes with local search moves to avoid getting stuck with bad %initial solutions. If there is any improvement in the objective function %value, then repeat step 3. 
%Else, STOP. 
%\vspace{0.3cm}
%\textbf{\hline}
%\vspace{0.4cm}
